...
export default class GuildCreate extends Event {
  ...
  public async run(guild): Promise<void> {
    ...
    const hook = new WebhookClient({ url: this.client.config.hooks.guildAdd.url });
        if (!hook) return;

        const embed = this.client.embed().setColor(this.client.color.main).setAuthor({ name: `${this.client.user.username} has been added to a guild.`, iconURL: guild.iconURL({}) }).setTitle(`${guild.name ? guild.name : 'unknowm'}`).setThumbnail(guild.iconURL({})).addFields([
            {
                name: 'Created On',
