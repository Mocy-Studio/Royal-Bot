import { RoyalBot, Context, Command } from '../../structures/index.js';
  ...
  public async run(client: RoyalBot, ctx: Context, args: string[]): Promise<void> {
    ...
    if (!args[0]) {
      ...
      const helpEmbed = embed
        ...
        .setAuthor({ name: `Help menu | ${ctx.guild.name}`, iconURL: this.client.user.avatarURL() })
        .setDescription(
          `Hey there! I'm ${this.client.user.username}, a moderation bot made with [Royal-Bot](https://github.com/Mocy-Studio/Royal-Bot) and Discord.js. You can use \`${prefix}help <command>\` to get more info on a command.`,
        )
        .setFooter({
          text: `@ Mocy Studio`,
      ...
      const embed = this.client.embed();
      const helpEmbed = embed.setColor(this.client.color.main).setAuthor({ name: `Help menu | ${command.name}`, iconURL: this.client.user.avatarURL() })
